...
TaskHandle_t uart_receive_task_handle;
...
/************************************************************
Get the heading data from UART
*************************************************************/
volatile uint16_t heading = 0;
volatile uint16_t receiving_heading = 0;

void uart_receive_update(void)
{
    uint8_t ch;

    while (boUART_getc(&ch))
    {
        if (ch >= '0' && ch <= '9')
        {
            receiving_heading =
                receiving_heading * 10 + (ch - '0');
        }
        else if (ch == '\n')
        {
            heading = receiving_heading;
            receiving_heading = 0;

            break;  // 1行完成したらUWB処理へ戻る
        }

        // '\r' は無視
    }
}
//static void uart_receive_task_function(void *pvParameter)
//{
//    uint8_t ch;
//    char buffer[16];
//    uint8_t index = 0;
...

{
  ...
  LEDS_ON(BSP_LED_0_MASK | BSP_LED_1_MASK | BSP_LED_2_MASK );
    ...
    //UNUSED_VARIABLE(xTaskCreate(uart_receive_task_function, "UART_RX", configMINIMAL_STACK_SIZE + 200, NULL, 2, &uart_receive_task_handle));
